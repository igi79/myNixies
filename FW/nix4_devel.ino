...
String inputString = "", serialString = "";
...
void serialScreen(){
    clr_nix(255);
    setString_nix(serialString);
}

void serialController(){
  if(stringComplete){
    char digit = (char)inputString.charAt(0);
    if(digit == 32 || (digit >= 48 && digit <= 57)){
      serialString = inputString.substring(0,5);
    } else {
      switch(digit){
        case 'm':
          dayScreen = inputString.substring(1).toInt();
          break;
        case 'l':
          uint8_t pinstate = inputString.substring(1).toInt();
          digitalWrite(LED, pinstate);
          break;          
        case 's':
          uint8_t what = inputString.charAt(1);
          unsigned int newValue = inputString.substring(2).toInt();
          DateTime newtime;          
          newtime.setyear(n.year());
          newtime.setmonth(n.month());
          newtime.setday(n.day());
          newtime.sethour(n.hour());
          newtime.setminute(n.minute());          
          newtime.setsecond(n.second());          
          switch(what){
            case '5':
              newtime.setyear(newValue);
              break;
            case '4':
              newtime.setmonth(newValue);
              break;
            case '3':
              newtime.setday(newValue);
              break;
            case '2':
              newtime.sethour(newValue);
              break;          
            case '1':
              newtime.setminute(newValue);
              break;
            case '0':
              newtime.setsecond(newValue);
              break;              
            default:
              break;
            }
...
void view(){
    ...
    if(sleep && (millis() - lastrefresh >= 500 ||forcerefresh)){
      clr_nix(255);
      digitalWrite(LED, ledtik == 1 ? HIGH : LOW);
      ledtik = ledtik == 1 ? 0 : 1;
      lastrefresh = millis();
      forcerefresh = false;
      return;
...
void setString_nix(String value){
  uint8_t l = value.length();
  for(uint8_t i = 0; i <= 5; i++){
    char digit = (char)value.charAt(i);
    nix[i] = digit == 32 ? 255 : (int)(digit - 48);
  }
}
